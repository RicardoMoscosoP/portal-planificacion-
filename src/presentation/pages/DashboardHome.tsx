...
export default function DashboardHome({ data, q, onNavCapacidad, onNav }: Props) {
            ...
            {visibleStakeholders.map(stakeholder => {
              ...
              return (
                <div key={stakeholder.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#fff', border: '1px solid var(--border)', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', minWidth: 180 }}>
                  <div style={{ width: 42, height: 42, borderRadius: '50%', background: '#1B30CC', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, flexShrink: 0 }}>
                    {stakeholder.nombre.split(' ').filter(Boolean).map(w => w[0]).join('').toUpperCase().slice(0, 2)}
                  </div>
                  <div style={{ minWidth: 0 }}>
